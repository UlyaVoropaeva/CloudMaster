...
/**
 * Класс объектов сообщений для файлов размером больше
 * CONST_FRAG_SIZE .
 */
public class FileFragmentMessage extends AbstractMessage {
    ...
    private FileInfo toDirectoryItem;
    ...
    public FileFragmentMessage(FileInfo toDirectoryItem, FileInfo item, long fullFileSize,
        ...
        fragName = constructFileFragName(item.getFullFilename(), currentFragNumber, totalFragsNumber);
        //составляем имя временной папки в директории назначения для хранения фрагментов файла
        toTempDirName = constructTempDirectoryName(item.getFullFilename(), fullFileSize);
